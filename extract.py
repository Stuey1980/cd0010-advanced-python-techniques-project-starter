...
def load_neos(neo_csv_path):
    ...
    with open(neo_csv_path) as f:
        ...
        # Iterate through each row and pass the neo with headers for all columns
        for elem in reader:
            # The class will extract the required entries and make an object
            elem["pdes"] = NearEarthObject(pdes=elem["pdes"], name=elem["name"],
                                           diameter=elem["diameter"], pha=elem["pha"])
...
def load_approaches(cad_json_path):
    ...
    for content in contents["data"]:
        ...
        content[0] = CloseApproach(des=content[0], cd=content[3], dist=content[4], v_rel=content[7])
